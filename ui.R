...
# ui.R
#
# This script defines the user interface (UI) for the Shiny application.
# It controls the layout and appearance of the app.
# ===================================================================
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    selectInput("provinsi", "Pilih Provinsi:", choices = provinsi_choices, selected = "SEMUA PROVINSI")
  ),
  ...
  dashboardBody(
    ...
    tabsetPanel(
      tabPanel(
        title = tagList(icon("chart-pie"), "Ringkasan Nasional"),
        fluidRow(
        ...
        fluidRow(
          box(
            title = "Grafik Jumlah Kejadian Bencana per Tahun", width = 12, status = "primary", solidHeader = TRUE,
      ...
      tabPanel(
        title = tagList(icon("globe"), "Peta Interaktif"),
      ...
      tabPanel(
        title = tagList(icon("cloud-sun"), "Analisis Iklim"),
      ...
      tabPanel(
        title = tagList(icon("chart-line"), "Analisis Statistik"),
        fluidRow(
          box(
            title = "Input Variabel", width = 4, status = "primary", solidHeader = TRUE,
          ...
          box(
            title = "Plot & Hasil", width = 8, status = "info", solidHeader = TRUE,
      ...
      tabPanel(
        title = tagList(icon("clock"), "Forecasting"),
        box(
          title = "Pengaturan Prediksi", width = 12, status = "info", solidHeader = TRUE,
          sliderInput("tahun_prediksi", "Jumlah Tahun Prediksi ke Depan:", min = 1, max = 10, value = 5)
        ),
        box(
          title = "Prediksi Jumlah Bencana", width = 12, status = "primary", solidHeader = TRUE,
