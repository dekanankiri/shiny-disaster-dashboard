              ...
              div(class="landing-page-content",
                  fluidRow(
                    column(width = 12, align="center",
                           ...
                           br(),
                           tags$img(src = "screenshot_peta.png", width = "80%", style="border-radius: 8px; box-shadow: 0 8px 16px rgba(0,0,0,0.15);"), 
                           br(), br(), br(),
      ...
      tabItem(tabName = "metadata",
              ...
              fluidRow(
                box(title = "Informasi Data", width = 12, solidHeader = TRUE, status = "primary",
                    ...
                    h4("Sumber Data"),
                    tags$ul(
                      tags$li("Data Kejadian Bencana & Iklim: `DataKomstat_Gabung.xlsx`"),
                      tags$li("Data Agregat Provinsi: `DataKomstat_Provinsi.xlsx`"),
  ...
  output$stat_output <- renderUI({
    ...
    if (input$run_cor) {
      if(var(data_filtered()[[input$xvar]], na.rm=TRUE) > 0 && var(data_filtered()[[input$yvar]], na.rm=TRUE) > 0) {
        ...
        tags_list <- append(tags_list, list(
          ...
          p(strong("Koefisien Korelasi (r): "), round(cor_test$estimate, 3)),
          p(strong("P-value: "), format.pval(cor_test$p.value, digits = 3, eps = 0.001)),
          hr()
        ))
      }
    }
    ...
    if (input$run_lm) {
      ...
      tags_list <- append(tags_list, list(
        ...
        p(strong("R-squared: "), round(summary_lm$r.squared, 3)),
